...
class AiterCommunicator:
    ...
    def _get_or_import(self, inp: torch.Tensor):
        ...
        rank = self._shmem.cur_rank
        _dbg(f"as_symmetric: ptr={ptr:#x} shape={tuple(inp.shape)} dtype={inp.dtype} numel={inp.numel()} device={inp.device}", rank=rank)
        sym_tensor = self._shmem.as_symmetric(inp)
        assert self._shmem.is_symmetric(sym_tensor), (
        ...
        _dbg(f"all_reduce_preamble: sym shape={tuple(sym_tensor.shape)} dtype={sym_tensor.dtype}", rank=rank)
        workspace = self._shmem.ccl.all_reduce_preamble(
            sym_tensor, sym_tensor,
        )
        _dbg(f"all_reduce_preamble OK", rank=rank)
    ...
    def _all_reduce_zerocopy(self, inp: torch.Tensor) -> torch.Tensor:
        ...
        rank = self._shmem.cur_rank
        _dbg(f"all_reduce zerocopy: shape={tuple(sym_tensor.shape)} dtype={sym_tensor.dtype} ptr={sym_tensor.data_ptr():#x}", rank=rank)
        workspace = self._shmem.ccl.all_reduce(
            sym_tensor, sym_tensor, workspace=workspace
        )
        _dbg(f"all_reduce zerocopy OK", rank=rank)
    ...
    def _get_buffers(self, shape, dtype):
        ...
        if self._buf_shape != shape or self._buf_dtype != dtype:
            ...
            _dbg(f"_get_buffers OK: inp_ptr={self._input_buf.data_ptr():#x} out_ptr={self._output_buf.data_ptr():#x}", rank=rank)
    ...
    def _all_reduce_copy(self, inp: torch.Tensor) -> torch.Tensor:
        ...
        if self._workspace is None:
            _dbg(f"all_reduce_preamble copy: shape={tuple(input_buf.shape)} dtype={input_buf.dtype}", rank=rank)
            self._workspace = self._shmem.ccl.all_reduce_preamble(
                input_buf, input_buf,
            )
            _dbg(f"all_reduce_preamble copy OK", rank=rank)
        _dbg(f"all_reduce copy: shape={tuple(input_buf.shape)} ptr={input_buf.data_ptr():#x}", rank=rank)
        self._workspace = self._shmem.ccl.all_reduce(
            input_buf, input_buf, workspace=self._workspace
        )
        _dbg(f"all_reduce copy OK", rank=rank)
    ...
    def all_reduce(self, inp: torch.Tensor) -> torch.Tensor:
        ...
        _dbg(f"all_reduce: path={self._allocator_type} inp shape={tuple(inp.shape)} dtype={inp.dtype} ptr={inp.data_ptr():#x} contiguous={inp.is_contiguous()}", rank=self._shmem.cur_rank)
